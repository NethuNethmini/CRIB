// Code generated by protoc-gen-ts_proto. DO NOT EDIT.
// versions:
//   protoc-gen-ts_proto  v2.6.1
//   protoc               unknown
// source: crib/user/v1/bank_user.proto

/* eslint-disable */
import { BinaryReader, BinaryWriter } from "@bufbuild/protobuf/wire";

export const protobufPackage = "crib.user.v1";

/** BankUser defines the BankUser message. */
export interface BankUser {
  bankId: string;
  bankName: string;
  licenseNumber: string;
  isActive: boolean;
  /** ContactInfo contact_info = 5; */
  walletAddress: string;
}

function createBaseBankUser(): BankUser {
  return { bankId: "", bankName: "", licenseNumber: "", isActive: false, walletAddress: "" };
}

export const BankUser: MessageFns<BankUser> = {
  encode(message: BankUser, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.bankId !== "") {
      writer.uint32(10).string(message.bankId);
    }
    if (message.bankName !== "") {
      writer.uint32(18).string(message.bankName);
    }
    if (message.licenseNumber !== "") {
      writer.uint32(26).string(message.licenseNumber);
    }
    if (message.isActive !== false) {
      writer.uint32(32).bool(message.isActive);
    }
    if (message.walletAddress !== "") {
      writer.uint32(42).string(message.walletAddress);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): BankUser {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBankUser();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag !== 10) {
            break;
          }

          message.bankId = reader.string();
          continue;
        }
        case 2: {
          if (tag !== 18) {
            break;
          }

          message.bankName = reader.string();
          continue;
        }
        case 3: {
          if (tag !== 26) {
            break;
          }

          message.licenseNumber = reader.string();
          continue;
        }
        case 4: {
          if (tag !== 32) {
            break;
          }

          message.isActive = reader.bool();
          continue;
        }
        case 5: {
          if (tag !== 42) {
            break;
          }

          message.walletAddress = reader.string();
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): BankUser {
    return {
      bankId: isSet(object.bankId) ? globalThis.String(object.bankId) : "",
      bankName: isSet(object.bankName) ? globalThis.String(object.bankName) : "",
      licenseNumber: isSet(object.licenseNumber) ? globalThis.String(object.licenseNumber) : "",
      isActive: isSet(object.isActive) ? globalThis.Boolean(object.isActive) : false,
      walletAddress: isSet(object.walletAddress) ? globalThis.String(object.walletAddress) : "",
    };
  },

  toJSON(message: BankUser): unknown {
    const obj: any = {};
    if (message.bankId !== "") {
      obj.bankId = message.bankId;
    }
    if (message.bankName !== "") {
      obj.bankName = message.bankName;
    }
    if (message.licenseNumber !== "") {
      obj.licenseNumber = message.licenseNumber;
    }
    if (message.isActive !== false) {
      obj.isActive = message.isActive;
    }
    if (message.walletAddress !== "") {
      obj.walletAddress = message.walletAddress;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<BankUser>, I>>(base?: I): BankUser {
    return BankUser.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<BankUser>, I>>(object: I): BankUser {
    const message = createBaseBankUser();
    message.bankId = object.bankId ?? "";
    message.bankName = object.bankName ?? "";
    message.licenseNumber = object.licenseNumber ?? "";
    message.isActive = object.isActive ?? false;
    message.walletAddress = object.walletAddress ?? "";
    return message;
  },
};

type Builtin = Date | Function | Uint8Array | string | number | boolean | undefined;

export type DeepPartial<T> = T extends Builtin ? T
  : T extends globalThis.Array<infer U> ? globalThis.Array<DeepPartial<U>>
  : T extends ReadonlyArray<infer U> ? ReadonlyArray<DeepPartial<U>>
  : T extends {} ? { [K in keyof T]?: DeepPartial<T[K]> }
  : Partial<T>;

type KeysOfUnion<T> = T extends T ? keyof T : never;
export type Exact<P, I extends P> = P extends Builtin ? P
  : P & { [K in keyof P]: Exact<P[K], I[K]> } & { [K in Exclude<keyof I, KeysOfUnion<P>>]: never };

function isSet(value: any): boolean {
  return value !== null && value !== undefined;
}

export interface MessageFns<T> {
  encode(message: T, writer?: BinaryWriter): BinaryWriter;
  decode(input: BinaryReader | Uint8Array, length?: number): T;
  fromJSON(object: any): T;
  toJSON(message: T): unknown;
  create<I extends Exact<DeepPartial<T>, I>>(base?: I): T;
  fromPartial<I extends Exact<DeepPartial<T>, I>>(object: I): T;
}
